...
static uint64_t hpet_read(size_t reg) {
  return *(volatile uint64_t *) phys_to_virt(hpet_base + reg);
}

static void hpet_write(size_t reg, uint64_t val) {
  *(volatile uint64_t *) phys_to_virt(hpet_base + reg) = val;
}
...
void hpet_sleep(size_t nanoseconds) {
  size_t target = hpet_read(HPET_MAIN_COUNTER) + (nanoseconds * 1000000) / hpet_period;
