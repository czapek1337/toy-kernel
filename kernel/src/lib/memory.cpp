...
uint64_t quick_memcpy(uint8_t *dest, const uint8_t *src, uint64_t size) {
...
uint64_t quick_memset(uint8_t *dest, uint8_t value, uint64_t size) {
    auto block_count = size / 8;
...
extern "C" void *memcpy(uint8_t *dest, const uint8_t *src, uint64_t size) {
    ...
    if (size > 0) {
        // dest += original_size - size;
        // src += original_size - size;
